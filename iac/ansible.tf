...
resource "null_resource" "ansible_connection_wait" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -u ${var.ssh_user} ../ansible/playbooks/connection-wait.yml -e target=all"
    environment = {
...
resource "null_resource" "ansible_install_kubernetes" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -u ${var.ssh_user} ../ansible/playbooks/install-kubernetes.yml -e target=all"
    environment = {
      ANSIBLE_CONFIG = "${var.ansible_cfg}"
    }
  }
